...
exports.postUrl = async (req, res) => {
    ...
    // check if slug is provided, create new one if not
    if (!slug) {
        slug = nanoid(5);
    }
    ...
    if (!foundSlug || foundSlug.length === 0) {
        ...
        res.status(200).json({ message: "URL generated", body: response });
    } else {
        res.status(409).json({ message: "URL exists already", body: { slug: "", url: "" } });
    }
}